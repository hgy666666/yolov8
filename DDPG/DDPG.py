...
STATE_DIM = 25 # 25个状态变量
ACTION_DIM = 7 # 7个动作变量
ACTION_BOUND = [30,1,1,1,1,1,1]
...
class Critic(object):
    ...
    def learn(self, s, a, r, s_, ISW,i):
        _, abs_td, los = self.sess.run([self.train_op, self.abs_td, self.loss], feed_dict={S: s, self.a: a, R: r, S_: s_, self.ISWeights: ISW})
        ls.append(los)
...
for i_episode in range(MAX_EPISODES):
    ...
    if h == 0:
        flag = 1
        print('i_episode=', i_episode, 'ep=', ep)
        ep_r.append(ep)
        u1x = 420
        u1y = 670
        u2x = 360
    ...
    a = np.clip(np.random.normal(a, var), [0, 0, 0, 0, 0, 0, 0],
                [29.9, 1, 1, 1, 1, 1, 1])  # add randomness to action selection for exploration
    u1x -= 2.11
    u1y -= 4.53
    u2x += 2.11
    u2y -= 4.53

    s_, r ,done= env.step(s,a,h,u1x,u1y,u2x,u2y,flag)
    ep += r
...
for i in range(0,len(ep_r)):
    ep_r[i] = ep_r[i] * 100

print('collection',coll)
wb = openpyxl.load_workbook('DDPG.xlsx')  # 读取excel表格
ws = wb['Sheet1']
for x in [ep_r[1::]]:
    ws.append(x)
rnd = list(range(1,len(ep_r)))
for x in [rnd]:
    ws.append(x)
wb.save('DDPG.xlsx')

plt.plot(range(1,len(ep_r)), ep_r[1::])
plt.xlabel("Training Step")
plt.ylabel("Reward")
plt.show()

plt.plot(range(1,len(ls)), ls[1::])
plt.ylabel('Loss')
plt.xlabel('training steps')
plt.show()

wa = openpyxl.load_workbook('loss.xlsx')  # 读取excel表格
wl = wa['Sheet1']
for i in range(1,len(ls)):
    wl.cell(i,1).value = ls[i]
    wl.cell(i,2).value = i
wa.save('loss.xlsx')